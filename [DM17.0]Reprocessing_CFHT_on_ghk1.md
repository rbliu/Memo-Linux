...
```
In some new versions of DM stack, the reportPatches step may also write some extra rows to patches.txt. So, double check patches_*.txt and make sure they only contain useful rows like:
