...
ssh to `ghk1` and setup DMstack. On `ghk1`, we have a `v14.0` version of DMstack installed.
...
source /export/rliu/DM_14_0/loadLSST.bash
setup lsst_distrib
```
...
All you need to do is to create a soft link to `/export/rliu/refcats/htm_baseline` in the `input` directory:
```
cd input
