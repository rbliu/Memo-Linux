...
* `detectCoaddSources.py` detects sources in the coadded images. Repeat this step for all bands you have coadded.
* `mergeCoaddDetections.py` merges the detected sources from multi-band. This step also generate consistent catalogs, the catalogs in multi-band with same rows. Connect all bands from the previous step with `^` in the `--id` option.
* `deblendCoaddSources.py` deblends coadded sources. Again, you need to repeat this step for all band.
* `measureCoaddSources.py` measures shapes and fluxes of the detected sources.
* `mergeCoaddMeasurements.py` sets the best measured band as the reference band for each source, which will be used in the following `forcedPhotometry` step. Connect all bands from the previous step with `^` in the `--id` option.
