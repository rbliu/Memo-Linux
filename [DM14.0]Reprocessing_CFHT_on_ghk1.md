...
* `detectCoaddSources.py` detects sources in the coadded images.
* `mergeCoaddDetections.py` merges the detected sources from multi-band. This step also generate consistent catalogs, the catalogs in multi-band with same rows.
* `deblendCoaddSources.py` deblends coadded sources.
* `measureCoaddSources.py` measures shapes and fluxes of the detected sources.
* `mergeCoaddMeasurements.py` sets the best measured band as the reference band for each source, which will be used in the following `forcedPhotometry` step.
